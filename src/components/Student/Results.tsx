              ...
              <div key={result._id} className="border border-gray-200 rounded-lg p-3 sm:p-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 space-y-2 sm:space-y-0">
                  ...
                  <div className="text-right sm:text-center">
                    <div className={`text-2xl font-bold ${getGradeColor((result.score / 30) * 100)}`}>
                      {getGrade((result.score / 30) * 100)}
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 text-xs sm:text-sm">
                  <div className="flex items-center">
                    ...
                    <span>Score: {result.score}/{30}</span>
                  </div>
                  <div className="flex items-center">
                    <TrendingUp className="h-4 w-4 mr-1 text-green-500" />
                    <span>Percentage: {((result.score / 30) * 100).toFixed(1)}%</span>
                  </div>
                  <div className="flex items-center">
                ...
                {rankings.map((ranking, index) => (
                  <tr key={ranking._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-2 sm:py-3 px-2 sm:px-4">
                      <span className={`font-medium ${getGradeColor((ranking.totalScore/(ranking.testsCount*30))*100)}`}>
                        {(ranking.totalScore/(ranking.testsCount*30)*100).toFixed(1)}%
                      </span>
                    </td>
                  </tr>
                ))}
