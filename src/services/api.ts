const API_BASE_URL = 'https://backend-2-mpen.onrender.com/api'
class ApiService {
  ...
  // Admin - Users
  async createUser(userData: { name: string; regno: string; password: string }) {
  ...
  async createUsers( name: string, regno: string, password: string ) {
    const response = await fetch(`${API_BASE_URL}/admin/users/reg`, {
      method: 'POST',
      headers: this.getHeaders(),
